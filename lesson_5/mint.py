import asyncio
import hashlib

from pytoniq import LiteBalancer, WalletV4R2, Contract
from pytoniq_core import StateInit, HashMap, begin_cell
from codes import jetton_minter_code, jetton_wallet_code
from schemes import JettonMinterData, JettonWalletData
from secret import mnemo


async def get_wallet(client):
    return await WalletV4R2.from_mnemonic(client, mnemo)


async def get_minter(client):

    def key_serializer(key: str):
        return int.from_bytes(hashlib.sha256(key.encode()).digest(), 'big')

    def value_serializer(src: str, dest: "Builder"):
        return dest.store_ref(begin_cell().store_uint(0, 8).store_snake_string(src).end_cell())

    content = HashMap(256, key_serializer=key_serializer, value_serializer=value_serializer)

    IMAGE = ''

    content.set('name', 'TEST COIN')
    content.set('description', 'very long description of the coin')
    # content.set('image', 'https://raw.githubusercontent.com/yungwine/pyton-lessons/master/lesson-6/image.webp')
    content.set('image_data', IMAGE)
    content.set('symbol', 'TST')

    content_cell = begin_cell().store_uint(0, 8).store_dict(content.serialize()).end_cell()
    # content_cell = begin_cell().store_uint(1, 8).store_string('https://...').end_cell()

    jetton_minter_data = JettonMinterData(
        0,
        'UQCPCZU37-aComPLgaQBcOkevn4x5GJHSfZsFt6eF9DpHZH9',
        content_cell,
        jetton_wallet_code
    )

    state_init = StateInit(code=jetton_minter_code, data=jetton_minter_data.serialize())

    jetton = await Contract.from_state_init(client, 0, state_init)
    return jetton


async def deploy_minter(client):
    jetton = await get_minter(client)
    wallet = await get_wallet(client)
    await wallet.transfer(jetton.address, 5 * 10 ** 7, state_init=jetton.state_init)


def get_mint_body(address: str, ton_amount: int, jetton_amount: int):
    return (begin_cell()
            .store_uint(21, 32)
            .store_uint(0, 64)
            .store_address(address)  # mint to
            .store_coins(ton_amount)  # nano ton
            .store_ref(
                begin_cell()
                .store_uint(0x178d4519, 32)
                .store_uint(0, 64)
                .store_coins(jetton_amount)  # nano jettons
                .store_address(None)
                .store_address(None)
                .store_coins(0)
                .store_bit_int(0)
                .end_cell()
            )
            .end_cell()
            )


async def mint(client):
    body = get_mint_body('UQCPCZU37-aComPLgaQBcOkevn4x5GJHSfZsFt6eF9DpHZH9', 3 * 10**7, 1000 * 10**9)
    wallet = await get_wallet(client)
    jetton = await get_minter(client)
    await wallet.transfer(jetton.address, amount=5 * 10 ** 7, body=body)


def get_change_owner_body(new_address: str):
    return begin_cell().store_uint(3, 32).store_uint(0, 64).store_address(new_address).end_cell()


async def change_owner(client):
    body = get_change_owner_body('EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c')
    wallet = await get_wallet(client)
    jetton = await get_minter(client)
    await wallet.transfer(jetton.address, amount=2 * 10 ** 7, body=body)


async def transfer(client):
    body = (begin_cell()
            .store_uint(0x0f8a7ea5, 32)
            .store_uint(0, 64)
            .store_coins(250 * 10**9)  # jetton amount
            .store_address('EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c')  # destination
            .store_address('UQCPCZU37-aComPLgaQBcOkevn4x5GJHSfZsFt6eF9DpHZH9')  # response
            .store_bit_int(0)
            .store_coins(1)  # forward amount
            .store_bit_int(1)
            .store_ref(
                begin_cell()
                .store_uint(0, 32)
                .store_snake_string('long comment')
                .end_cell()
            )
            .end_cell()
    )
    wallet = await get_wallet(client)
    await wallet.transfer('EQDceelhB7BNJWyxekP556Ze4cu4y6EepkPm6em9NDCNihT3', amount=10**8, body=body)


async def burn(client):
    body = (begin_cell()
            .store_uint(0x595f07bc, 32)
            .store_uint(0, 64)
            .store_coins(100 * 10**9)  # jetton amount
            .store_address('UQCPCZU37-aComPLgaQBcOkevn4x5GJHSfZsFt6eF9DpHZH9')  # response
            )

    wallet = await get_wallet(client)
    await wallet.transfer('EQDceelhB7BNJWyxekP556Ze4cu4y6EepkPm6em9NDCNihT3', amount=10**8, body=body)


async def main():
    client = LiteBalancer.from_mainnet_config(1)

    await client.start_up()

    await burn(client)

    await client.close_all()

if __name__ == '__main__':
    asyncio.run(main())
